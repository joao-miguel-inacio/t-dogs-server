...
router.get("/", isAuthenticated, async (req, res, next) => {
  ...
  try {
    ...
    const foundUser = await Buyer.findById(foundUserId) || await Owner.findById(foundUserId);
...
router.put("/", isAuthenticated, async (req, res, next) => {
  ...
  try {
    ...
    const updatedUser = await Buyer.findByIdAndUpdate(req.body, { new: true }) || await Owner.findByIdAndUpdate(req.body, { new: true }); 
    // using descriptors: if uncommenting the next line, please comment the line above
